...
在`Golang`中，`interface`的作用类似于电脑主板上的接口。它是一种类型，用于定义一组方法签名。一个**实现了这组方法的具体类型**被称为这个`interface`的**实现类型**。就像`USB`接口一样, `USB`定义了插口大小, 金属引脚数量等; 任何品牌的鼠标,键盘等外部组件只要按照`USB`规定的插口大小,引脚数量进行制作,就能插到USB接口上并使用。
